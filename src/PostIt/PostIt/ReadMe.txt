================================================================================
    MFC 라이브러리 : PostIt 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 PostIt 응용 프로그램을 
만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램
작성을 위한 기본 구조를 제공합니다.

이 파일에는 PostIt 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
PostIt.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CPostItApp 응용 프로그램
    클래스를 선언합니다.

PostIt.cpp
    응용 프로그램 클래스 CPostItApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
PostItDoc.h, PostItDoc.cpp - 문서
    이 파일에는 CPostItDoc 클래스가 들어 있습니다. 이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CPostItDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.
    이 문서에는 다음 문자열이 포함됩니다.
        파일 확장명:      txt
        파일 형식 ID:        PostIt.Document
        주 프레임 캡션:  PostIt
        문서 형식 이름:       PostIt
        필터 이름:         PostIt Files (*.txt)
        파일의 새 약식 이름: PostIt
        파일 형식의 긴 이름: PostIt.Document

PostItView.h, PostItView.cpp - 문서 뷰
    이 파일에는 CPostItView 클래스가 들어 있습니다.
    CPostItView 개체는 CPostItDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 PostIt.pch와
